...
// 选择下一座要处理的城市（当前已知距离最短且未访问）
city *nextToVisit() {
    city *res = NULL;
    for (int i = 0; i < nCities; i++) {
        if (cityList[i]->visited == 0) {
            if (res == NULL || cityList[i]->timeToGetHere < res->timeToGetHere) {
                res = cityList[i];
            }
        }
    }
...
// 用当前城市去尝试更新所有相邻城市的最短距离，并记录路径来源
void visit (city *c) {

    for (int i = 0; i < c->nLinks; i++) {
        city *neighbour = c->linkCity[i];
        int timeToReachNeighbour = 0;

        if (neighbour->visited == 0) {
            timeToReachNeighbour = c->timeToGetHere + c->linkMinutes[i];
            
            if (timeToReachNeighbour < neighbour->timeToGetHere) {
                neighbour->timeToGetHere = timeToReachNeighbour;
...
// 根据 cityBeforeThis 指针，把最终路径从起点到终点打印出来
...
void dijkstra (char *startName, char *endName) {
    ...
    // Prepare to run Dijkstra;
    prepareForDijkstra();

    // Look up start and end cities
    ...
    printf("Time to reach %s is: %d\n", end->name, end->timeToGetHere);
    traceBack(end);
