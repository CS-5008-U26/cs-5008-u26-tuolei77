...
void heapifyUp(int *a, int n, int here) {
    ...
    // if here is not root and here is larger than its parent, do the swap
...
void heapifyDown(int *a, int n, int here) {
    ...
    // find largest value of the three
    // when here is not a leaf node
    int largest = here;
    if (child1 < n && a[child1] > a[largest]) largest = child1;
    if (child2 < n && a[child2] > a[largest]) largest = child2;

    // swap current value with largest if largest is not current value
	if (largest != here) {
        int temp = a[here];
        a[here] = a[largest];
        a[largest] = temp;
...
void sortarray (int a[], int n) {
    ...
    // starting with element 1 up to n-1, do heapifyUp on each node
    for (int i = 1; i < n; i++) {
        heapifyUp(a, n, i);
    }

    // repeatedly exchange the root with the last element, 
    // reduce the number of elements by 1, and do heapifyDown on the root
    while (n > 1) {
        int temp = a[0];
        a[0] = a[n - 1];
        a[n - 1] = temp;
