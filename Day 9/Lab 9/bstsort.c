...
tNode *insertBST(tNode *t, int x) {
     ...
     * Create a new leaf node here and return it so the caller
     * can attach it as a child. */
    if (t == NULL) {
     ...
     * recurse into the left subtree, and reattach whatever
     * insertBST returns (either an existing subtree root,
     * or a newly created node if we hit a NULL spot). */    
    if (x <= t->value) {
    ...
    /* Return the (possibly unchanged) root of this subtree
