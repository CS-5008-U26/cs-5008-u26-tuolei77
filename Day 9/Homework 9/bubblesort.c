...
void sortarray (int a[], int n) {
    ...
    /* range [0, i] is the unsorted part */
    for (int i = n - 1; i >= 1; i--) {        
        int numExchange = 0;

        /* Compare each element to its neighbour and swap if out of order */
        /* The largest value in this round will be in place */
        for (int j = 0; j < i; j++) {
            if (a[j] > a[j + 1]) {
                int temp = a[j];
                a[j] = a[j + 1];
        ...
        /* No exchange for the turn and the array is fully sorted, exit earlier */
        if (numExchange == 0) return;
    }
}
