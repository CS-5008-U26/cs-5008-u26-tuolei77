...
/* A helper function that merges two arrays */
int *merge(int *a, int *b, int an, int bn) {
    int *result = malloc ((an + bn) * sizeof(int));
    int ai = 0;
    int bi = 0;
    int i = 0;
...
int * merge_sort(int a[], int n) {
    ...
    int *firsthalf = merge_sort (a, middle);
    int *secondhalf = merge_sort (a + middle, n - middle);

    int *result = merge(firsthalf, secondhalf, middle, n - middle);
    /* Free the memory only when it's not from base case */
    if (middle > 1) {
        free(firsthalf);
    }
    if (n - middle > 1) {
...
void sortarray (int a[], int n) {
    ...
    /* Copy the result back into the original array a[] */
    for (int i = 0; i < n; i++) {
        a[i] = result[i];
    }

    /* Free the memory for non base case */
    if (n > 1) {
        free(result);
    }
}
