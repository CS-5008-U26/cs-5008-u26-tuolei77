 ...
 * double linked list, and enables actions including size, delete,
...
/* Create a struct to store a doubly linked list node */
...
dNode *addToFront (city *x, dNode *head) {
...
dNode *addToEnd (city *x, dNode *head) {
...
dNode *getNode(int n, dNode *head) {
...
dNode *deleteNode(dNode *head, dNode *target) {
    ...
    /* skip over target going forward */
    if (target->prev != NULL) {
        target->prev->next = target->next;   
    } else {
    /* target was the head, so its next becomes the new head */
        head = target->next;   
    }

    if (target->next != NULL) {
        /* skip over target going backward */
...
dNode *moveToFront(int n, dNode *head) {

    dNode *target = getNode(n, head);
...
/* Read the first 20 cities from the file into a doubly-linked list */
...
int main () {
    ...
    dNode *cityList = readCityList("uscities.csv");
    ...
    while (1) {
        ...
        if (strcmp(buffer, "size") == 0) {
            ...
            if (num <= 0 || num > getLength(cityList)) {
                ...
                dNode *target = getNode(num - 1, cityList);
                city *c = (city *)target->data;
